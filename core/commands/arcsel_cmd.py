...
from core.arc_geom import arc_from_scl
from core.commands.base_command import BaseCommand

class ArcSCLCommand(BaseCommand):
    ...
    @property
    def prompt(self):
        if self._phase == "START": return "ARCO-SCL ▶ Especifique punto inicial:"
        elif self._phase == "CENTER": return "ARCO-SCL ▶ Especifique centro:"
        return "ARCO-SCL ▶ Especifique longitud de cuerda:"
    ...
    def on_left_click(self, world_pt):
        if self._phase == "START":
            ...
            self._center = world_pt; self._phase = "LENGTH"
    ...
    def on_enter(self, text: str = ""):
        if self._phase == "START":
            ...
            if pt: self._start = pt; self._phase = "CENTER"
        elif self._phase == "CENTER":
            pt = parse_coord_input(text, self._start)
            if pt: self._center = pt; self._phase = "LENGTH"
        elif self._phase == "LENGTH":
            try:
                self._create(float(text))
            except ValueError:
                self.log("Longitud inválida.")

    def _create(self, chord_length: float):
        res = arc_from_scl(self._start, self._center, chord_length)
        if res:
            self.doc.push_undo()
            self.doc.add_entity(ArcEntity(*res))
            self.log(f"Arco SCL creado (cuerda={chord_length:.2f}).")
        else:
    ...
    def draw_preview(self, painter, viewport):
        ...
        elif self._phase == "LENGTH" and self._start and self._center:
            r = math.hypot(self._start.x() - self._center.x(), self._start.y() - self._center.y())
            cx, cy = self._center.x(), self._center.y()
            sa = math.degrees(math.atan2(self._start.y() - cy, self._start.x() - cx))
